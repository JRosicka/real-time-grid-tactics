...
namespace Gameplay.UI {
    ...
    public class PathVisualizer : MonoBehaviour {
        ...
        /// </summary>
        public void Visualize(List<GridNode> path) {
            ClearPath();
            ...
            for (int i = 0; i < path.Count - 1; i++) {
                ...
                line.transform.position = GridController.GetWorldPosition(path[i].Location);
                
                // Rotate the line to link to the next cell in the path
                var previousAngle = currentAngle;
                currentAngle = PathfinderService.AngleBetweenCells(path[i].Location, path[i + 1].Location);
