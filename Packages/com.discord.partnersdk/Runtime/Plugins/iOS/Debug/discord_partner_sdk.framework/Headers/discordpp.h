...
class GuildMemberHandle {
    /// \cond
    mutable Discord_GuildMemberHandle instance_{};
    DiscordObjectState state_ = DiscordObjectState::Invalid;
    /// \endcond

public:
    /// \cond
    Discord_GuildMemberHandle* instance() const { return &instance_; }
    /// \endcond
    /// \cond
    explicit GuildMemberHandle(Discord_GuildMemberHandle instance, DiscordObjectState state);
    ~GuildMemberHandle();
    /// \endcond
    /// Move constructor for GuildMemberHandle
    GuildMemberHandle(GuildMemberHandle&& other) noexcept;
    /// Move assignment operator for GuildMemberHandle
    GuildMemberHandle& operator=(GuildMemberHandle&& other) noexcept;
    /// Uninitialized instance of GuildMemberHandle
    static const GuildMemberHandle nullobj;
    /// Returns true if the instance contains a valid object
    operator bool() const { return state_ != DiscordObjectState::Invalid; }

    /// Copy constructor for GuildMemberHandle
    GuildMemberHandle(const GuildMemberHandle& other);
    /// Copy assignment operator for GuildMemberHandle
    GuildMemberHandle& operator=(const GuildMemberHandle& other);
    ...
    std::optional<std::string> NickName() const;
...
class GuildHandle {
    /// \cond
    mutable Discord_GuildHandle instance_{};
    DiscordObjectState state_ = DiscordObjectState::Invalid;
    /// \endcond

public:
    /// \cond
    Discord_GuildHandle* instance() const { return &instance_; }
    /// \endcond
    /// \cond
    explicit GuildHandle(Discord_GuildHandle instance, DiscordObjectState state);
    ~GuildHandle();
    /// \endcond
    /// Move constructor for GuildHandle
    GuildHandle(GuildHandle&& other) noexcept;
    /// Move assignment operator for GuildHandle
    GuildHandle& operator=(GuildHandle&& other) noexcept;
    /// Uninitialized instance of GuildHandle
    static const GuildHandle nullobj;
    /// Returns true if the instance contains a valid object
    operator bool() const { return state_ != DiscordObjectState::Invalid; }

    /// Copy constructor for GuildHandle
    GuildHandle(const GuildHandle& other);
    /// Copy assignment operator for GuildHandle
    GuildHandle& operator=(const GuildHandle& other);

    /// \cond
    void Drop();
    /// \endcond

    std::vector<uint64_t> ChannelIds() const;

    std::optional<discordpp::GuildMemberHandle> GetGuildMemberHandle(uint64_t userId) const;

    std::vector<uint64_t> GuildMemberIds() const;

    std::optional<std::string> IconUrl() const;

    uint64_t Id() const;

    std::string Name() const;

    uint32_t PremiumTier() const;
...
public:
    ...
    /// \brief Returns true if the target user has played the game. Note, for Discord friends, this
...
public:
    ...
    using UpdateLobbyMemberCallback =
      std::function<void(discordpp::ClientResult result, uint64_t userId, uint64_t lobbyId)>;
    using LobbyActionCallback =
      std::function<void(discordpp::ClientResult result, uint64_t lobbyId)>;
    using PerformOnThreadWithStringCallback = std::function<void(std::string text)>;
    using UpdateUserApplicationProfileCallback =
    ...
    void AddOrUpdateLobbyMember(std::string const& token,
                                uint64_t userId,
                                uint64_t lobbyId,
                                std::unordered_map<std::string, std::string> const& metadata,
                                discordpp::LobbyMemberFlags flags,
                                discordpp::Client::UpdateLobbyMemberCallback callback,
                                std::optional<std::string> additionalName);

    void CreateNewLobby(std::string const& token, discordpp::Client::LobbyActionCallback callback);

    void DeleteLobby(std::string const& token,
                     uint64_t lobbyId,
                     discordpp::Client::LobbyActionCallback callback);

    void EditLobby(std::string const& token,
                   uint64_t lobbyId,
    ...
    std::optional<discordpp::GuildHandle> GetGuildHandle(uint64_t id) const;

    std::vector<uint64_t> GetGuildIds() const;
    ...
    void PerformOnThreadWithString_ForTest(
      ...
      discordpp::Client::PerformOnThreadWithStringCallback callback);

    void RemoveLobbyMember(std::string const& token,
                           uint64_t userId,
                           uint64_t lobbyId,
                           discordpp::Client::UpdateLobbyMemberCallback callback);
    ...
    void UpdateUserApplicationProfile(std::string const& token,
                                      uint64_t applicationId,
                                      uint64_t userId,
                                      std::string const& providerIssuedUserId,
                                      std::string const& username,
                                      std::unordered_map<std::string, std::string> const& metadata,
                                      discordpp::Client::UpdateUserApplicationProfileCallback cb);
    ...
    void GetProvisionalTokenBotAPI(uint64_t applicationId,
                                   std::string const& botToken,
                                   std::string const& externalUserId,
                                   std::string const& preferredGlobalName,
                                   discordpp::Client::TokenExchangeCallback callback);
...
/// Converts a discordpp::LobbyMemberFlags to a string.
inline const char* EnumToString(discordpp::LobbyMemberFlags value)
{
    switch (value) {
    case discordpp::LobbyMemberFlags::None:
        return "None";
    case discordpp::LobbyMemberFlags::CanLinkLobby:
        return "CanLinkLobby";
    default:
        return "unknown";
    }
...
std::optional<discordpp::CallInfoHandle> ChannelHandle::GetCallInfoHandle() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_CallInfoHandle returnValueNative__;
    returnIsNonNull__ = Discord_ChannelHandle_GetCallInfoHandle(&instance_, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
...
const GuildMemberHandle GuildMemberHandle::nullobj{{}, DiscordObjectState::Invalid};
GuildMemberHandle::~GuildMemberHandle()
{
    if (state_ == DiscordObjectState::Owned) {
        Drop();
        state_ = DiscordObjectState::Invalid;
...
GuildMemberHandle::GuildMemberHandle(GuildMemberHandle&& other) noexcept
  : instance_(other.instance_)
  , state_(other.state_)
{
    other.state_ = DiscordObjectState::Invalid;
}
GuildMemberHandle& GuildMemberHandle::operator=(GuildMemberHandle&& other) noexcept
{
    if (this != &other) {
        if (state_ == DiscordObjectState::Owned) {
            Drop();
...
GuildMemberHandle::GuildMemberHandle(const GuildMemberHandle& other)
  : instance_{}
  , state_(DiscordObjectState::Invalid)
{
    if (other.state_ == DiscordObjectState::Owned) {
        Discord_GuildMemberHandle_Clone(&instance_, other.instance());

        state_ = DiscordObjectState::Owned;
    }
}
GuildMemberHandle& GuildMemberHandle::operator=(const GuildMemberHandle& other)
{
    if (this != &other) {
        if (state_ == DiscordObjectState::Owned) {
            Drop();
...
GuildMemberHandle::GuildMemberHandle(Discord_GuildMemberHandle instance, DiscordObjectState state)
  : instance_(instance)
  , state_(state)
{
}
void GuildMemberHandle::Drop()
{
    if (state_ != DiscordObjectState::Owned) {
        return;
    }
    Discord_GuildMemberHandle_Drop(&instance_);
    state_ = DiscordObjectState::Invalid;
}
std::optional<std::string> GuildMemberHandle::Avatar() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_String returnValueNative__;
    returnIsNonNull__ = Discord_GuildMemberHandle_Avatar(&instance_, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
...
std::optional<std::string> GuildMemberHandle::AvatarUrl() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_String returnValueNative__;
    returnIsNonNull__ = Discord_GuildMemberHandle_AvatarUrl(&instance_, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
...
bool GuildMemberHandle::Deaf() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildMemberHandle_Deaf(&instance_);
    return returnValue__;
}
bool GuildMemberHandle::IsGuest() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildMemberHandle_IsGuest(&instance_);
    return returnValue__;
}
bool GuildMemberHandle::Mute() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildMemberHandle_Mute(&instance_);
    return returnValue__;
}
std::optional<std::string> GuildMemberHandle::NickName() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_String returnValueNative__;
    returnIsNonNull__ = Discord_GuildMemberHandle_NickName(&instance_, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
...
bool GuildMemberHandle::SelfDeaf() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildMemberHandle_SelfDeaf(&instance_);
    return returnValue__;
}
bool GuildMemberHandle::SelfMute() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildMemberHandle_SelfMute(&instance_);
    return returnValue__;
}
const GuildHandle GuildHandle::nullobj{{}, DiscordObjectState::Invalid};
...
GuildHandle::GuildHandle(GuildHandle&& other) noexcept
  : instance_(other.instance_)
  , state_(other.state_)
{
    other.state_ = DiscordObjectState::Invalid;
}
GuildHandle& GuildHandle::operator=(GuildHandle&& other) noexcept
{
    if (this != &other) {
        if (state_ == DiscordObjectState::Owned) {
            Drop();
...
GuildHandle::GuildHandle(const GuildHandle& other)
  : instance_{}
  , state_(DiscordObjectState::Invalid)
{
    if (other.state_ == DiscordObjectState::Owned) {
        Discord_GuildHandle_Clone(&instance_, other.instance());

        state_ = DiscordObjectState::Owned;
    }
}
GuildHandle& GuildHandle::operator=(const GuildHandle& other)
{
    if (this != &other) {
        if (state_ == DiscordObjectState::Owned) {
            Drop();
...
GuildHandle::GuildHandle(Discord_GuildHandle instance, DiscordObjectState state)
  : instance_(instance)
  , state_(state)
{
}
void GuildHandle::Drop()
{
    if (state_ != DiscordObjectState::Owned) {
        return;
    }
    Discord_GuildHandle_Drop(&instance_);
    state_ = DiscordObjectState::Invalid;
}
std::vector<uint64_t> GuildHandle::ChannelIds() const
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_UInt64Span returnValueNative__;
    Discord_GuildHandle_ChannelIds(&instance_, &returnValueNative__);
    std::vector<uint64_t> returnValue__(returnValueNative__.ptr,
                                        returnValueNative__.ptr + returnValueNative__.size);
    Discord_Free(returnValueNative__.ptr);
    return returnValue__;
}
std::optional<discordpp::GuildMemberHandle> GuildHandle::GetGuildMemberHandle(uint64_t userId) const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_GuildMemberHandle returnValueNative__;
    returnIsNonNull__ =
      Discord_GuildHandle_GetGuildMemberHandle(&instance_, userId, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    discordpp::GuildMemberHandle returnValue__(returnValueNative__, DiscordObjectState::Owned);
    return returnValue__;
}
std::vector<uint64_t> GuildHandle::GuildMemberIds() const
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_UInt64Span returnValueNative__;
    Discord_GuildHandle_GuildMemberIds(&instance_, &returnValueNative__);
    std::vector<uint64_t> returnValue__(returnValueNative__.ptr,
                                        returnValueNative__.ptr + returnValueNative__.size);
    Discord_Free(returnValueNative__.ptr);
    return returnValue__;
}
std::optional<std::string> GuildHandle::IconUrl() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_String returnValueNative__;
    returnIsNonNull__ = Discord_GuildHandle_IconUrl(&instance_, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
...
uint64_t GuildHandle::Id() const
{
    assert(state_ == DiscordObjectState::Owned);
    uint64_t returnValue__;
    returnValue__ = Discord_GuildHandle_Id(&instance_);
    return returnValue__;
}
std::string GuildHandle::Name() const
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_String returnValueNative__;
    Discord_GuildHandle_Name(&instance_, &returnValueNative__);
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
                              returnValueNative__.size);
    Discord_Free(returnValueNative__.ptr);
    return returnValue__;
}
uint32_t GuildHandle::PremiumTier() const
{
    assert(state_ == DiscordObjectState::Owned);
    uint32_t returnValue__;
    returnValue__ = Discord_GuildHandle_PremiumTier(&instance_);
    return returnValue__;
}
bool GuildHandle::Unavailable() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_GuildHandle_Unavailable(&instance_);
    return returnValue__;
}
...
bool RelationshipHandle::HasPlayedGame() const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnValue__;
    returnValue__ = Discord_RelationshipHandle_HasPlayedGame(&instance_);
    return returnValue__;
}
...
void Client::AddOrUpdateLobbyMember(std::string const& token,
                                    uint64_t userId,
                                    uint64_t lobbyId,
                                    std::unordered_map<std::string, std::string> const& metadata,
                                    discordpp::LobbyMemberFlags flags,
                                    ...
                                    std::optional<std::string> additionalName)
{
    ...
    Discord_Client_UpdateLobbyMemberCallback callback__native =
      [](auto result, auto userId, auto lobbyId, void* userData__) {
          ...
          userData__typed->delegate(std::move(result__obj), userId, lobbyId);
      };
    Discord_String additionalName__str{};
    if (additionalName.has_value()) {
        additionalName__str.ptr = reinterpret_cast<uint8_t*>(additionalName->data());
        additionalName__str.size = additionalName->size();
    }
    Discord_Client_AddOrUpdateLobbyMember(
      &instance_,
      token__str,
      userId,
      lobbyId,
      ...
      callback__userData,
      (additionalName.has_value() ? &additionalName__str : nullptr));
}
void Client::CreateNewLobby(std::string const& token,
                            discordpp::Client::LobbyActionCallback callback)
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_String token__str{(uint8_t*)(token.data()), token.size()};
    ...
    Discord_Client_CreateNewLobby(
      &instance_, token__str, callback__native, Tcallback__UserData::Free, callback__userData);
}
void Client::DeleteLobby(std::string const& token,
                         uint64_t lobbyId,
                         discordpp::Client::LobbyActionCallback callback)
{
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_DeleteLobby(&instance_,
                               token__str,
                               lobbyId,
                               callback__native,
                               Tcallback__UserData::Free,
...
void Client::EditLobby(std::string const& token,
                       uint64_t lobbyId,
                       std::unordered_map<std::string, std::string> const& lobbyMetadata,
                       discordpp::Client::LobbyActionCallback callback)
{
    ...
    Discord_Client_EditLobby(&instance_,
                             token__str,
                             lobbyId,
...
void Client::ForceCrash_ForTest()
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_Client_ForceCrash_ForTest(&instance_);
}
...
std::optional<discordpp::GuildHandle> Client::GetGuildHandle(uint64_t id) const
{
    assert(state_ == DiscordObjectState::Owned);
    bool returnIsNonNull__;
    Discord_GuildHandle returnValueNative__;
    returnIsNonNull__ = Discord_Client_GetGuildHandle(&instance_, id, &returnValueNative__);
    if (!returnIsNonNull__) {
        return {};
    }
    discordpp::GuildHandle returnValue__(returnValueNative__, DiscordObjectState::Owned);
    return returnValue__;
}
std::vector<uint64_t> Client::GetGuildIds() const
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_UInt64Span returnValueNative__;
    Discord_Client_GetGuildIds(&instance_, &returnValueNative__);
    std::vector<uint64_t> returnValue__(returnValueNative__.ptr,
                                        returnValueNative__.ptr + returnValueNative__.size);
    Discord_Free(returnValueNative__.ptr);
    return returnValue__;
...
void Client::PerformOnThreadWithString_ForTest(
  std::string const& text,
  discordpp::Client::PerformOnThreadWithStringCallback callback)
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_String text__str{(uint8_t*)(text.data()), text.size()};
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)>>;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_PerformOnThreadWithStringCallback callback__native = [](auto text,
                                                                           void* userData__) {
        auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
        std::string text__str(reinterpret_cast<char*>(text.ptr), text.size);
        userData__typed->delegate(std::move(text__str));
        Discord_Free(text.ptr);
    };
    Discord_Client_PerformOnThreadWithString_ForTest(
      &instance_, text__str, callback__native, Tcallback__UserData::Free, callback__userData);
}
void Client::RemoveLobbyMember(std::string const& token,
                               uint64_t userId,
                               uint64_t lobbyId,
                               discordpp::Client::UpdateLobbyMemberCallback callback)
{
    ...
    Discord_Client_RemoveLobbyMember(&instance_,
                                     token__str,
                                     userId,
                                     lobbyId,
                                     callback__native,
...
void Client::UpdateUserApplicationProfile(
  std::string const& token,
  uint64_t applicationId,
  uint64_t userId,
  std::string const& providerIssuedUserId,
  ...
  discordpp::Client::UpdateUserApplicationProfileCallback cb)
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_String token__str{(uint8_t*)(token.data()), token.size()};
    Discord_String providerIssuedUserId__str{(uint8_t*)(providerIssuedUserId.data()),
                                             providerIssuedUserId.size()};
    ...
    Discord_Client_UpdateUserApplicationProfile(&instance_,
                                                token__str,
                                                applicationId,
                                                userId,
                                                providerIssuedUserId__str,
                                                username__str,
                                                metadata__convert.Properties,
                                                cb__native,
                                                Tcb__UserData::Free,
...
void Client::GetProvisionalTokenBotAPI(uint64_t applicationId,
                                       std::string const& botToken,
                                       std::string const& externalUserId,
                                       std::string const& preferredGlobalName,
                                       discordpp::Client::TokenExchangeCallback callback)
{
    assert(state_ == DiscordObjectState::Owned);
    Discord_String botToken__str{(uint8_t*)(botToken.data()), botToken.size()};
    Discord_String externalUserId__str{(uint8_t*)(externalUserId.data()), externalUserId.size()};
    Discord_String preferredGlobalName__str{(uint8_t*)(preferredGlobalName.data()),
                                            preferredGlobalName.size()};
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)>>;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_TokenExchangeCallback callback__native = [](auto result,
                                                               auto accessToken,
    ...
    Discord_Client_GetProvisionalTokenBotAPI(&instance_,
                                             applicationId,
                                             botToken__str,
                                             externalUserId__str,
                                             preferredGlobalName__str,
                                             callback__native,
                                             Tcallback__UserData::Free,
                                             callback__userData);
}
